...
# LEAST-SQUARES IMPLIED RETURNS ESTIMATOR WITH L2 REGULARIZATION

# In[7]:


def ls_implied_dif_expected_returns(returns_df, act_alloc, prtns, regularization_weight=0.0001):
    """
    Least squares optimization with L2 regularization
    ...
    - regularization_weight: L2 penalty weight (default 0.0001)
    """
    print(f'\nLeast-squares optimization with L2 regularization (weight={regularization_weight})')
    ...
    print(f'adjustment_contribution type: {type(adjustment_contribution)}')
    ...
    # Minimize sum of squared differences with L2 regularization
    # The regularization term adds a small penalty for each adjustment squared,
    # naturally preferring smaller, more distributed adjustments
    objective = cp.Minimize(cp.sum_squares(dif_prtn) + regularization_weight * cp.sum_squares(xxx))
    
    # Add constraint: sum of adjustments must be zero
    constraints = [cp.sum(xxx) == 0]
    ...
    print(f'  Portfolio matching term: {cp.sum_squares(dif_prtn).value:.6f}')
    print(f'  Regularization term: {(regularization_weight * cp.sum_squares(xxx)).value:.6f}')
    ...
    print(f'  RMS of adjustments: {np.sqrt(np.mean(xxx.value**2)):.6f}')
...
def woptimize(params={}):
    ...
    ls_implied_dif = np.zeros((len(returns_df.columns),len(Llist)))
    ls_implied_dif_df = pd.DataFrame(ls_implied_dif,columns=Llist,index=returns_df.columns)
    ...
    for lev in Llist:
        ...
        ls_implied_dif_df[lev] = ls_implied_dif_expected_returns(returns_df,act_alloc,prtns) 
    ...
    with pd.option_context('display.float_format', '{:,.5f}'.format):
        ...
        print(' ')
        print('Least-Squares Implied Expected Return Difference (L2 regularized)')
        print(ls_implied_dif_df)
    ...
    ls_adjustments = ls_implied_dif_df[median_lev].values
    ...
    econ_results = economic_rebalancing_analysis(
        xxx_adjustments=ls_adjustments,
    ...
    with pd.option_context('display.float_format', '{:,.4f}'.format,
                          ...
                          'display.width', None):
        print('\nDETAILED REBALANCING RECOMMENDATIONS:')
        print(econ_results.to_string(index=False))
        
        # Summary statistics
        trades_to_make = econ_results[econ_results['Should_Trade']]
        print(f'\nSUMMARY:')
        print(f'  Total assets: {len(econ_results)}')
        print(f'  Recommended trades: {len(trades_to_make)}')
        print(f'  Total turnover if all trades: {np.abs(econ_results["Trade_Size"]).sum():.4f}')
        print(f'  Turnover from recommended trades: {np.abs(trades_to_make["Trade_Size"]).sum():.4f}')
        print(f'  Transaction cost (all trades): {econ_results["Trade_Cost"].sum():.4f}')
        print(f'  Transaction cost (recommended): {trades_to_make["Trade_Cost"].sum():.4f}')
        
        # Show volatility insights
        print('\nVOLATILITY-NORMALIZED INSIGHTS:')
        print('(Adjustments divided by annual volatility - higher means more significant)')
        vol_norm_sorted = econ_results.nlargest(5, 'Vol_Norm_Adj')[['Asset', 'Vol_Norm_Adj', 'Annual_Vol', 'Implied_Adj']]
        print(vol_norm_sorted.to_string(index=False))
        ...
        comparison_df = pd.DataFrame({
            'Asset': returns_df.columns,
            'NM_Adjustment': nm_adjustments,
            'LS_Regularized': ls_adjustments,
            'Difference': ls_adjustments - nm_adjustments
        })
        print(comparison_df.to_string(index=False))
        
        print(f'\nAdjustment Statistics:')
        print(f'  Nelder-Mead:')
        print(f'    Range: [{nm_adjustments.min():.6f}, {nm_adjustments.max():.6f}]')
        print(f'    Std dev: {nm_adjustments.std():.6f}')
        print(f'    RMS: {np.sqrt(np.mean(nm_adjustments**2)):.6f}')
        print(f'  Least Squares (L2 regularized):')
        print(f'    Range: [{ls_adjustments.min():.6f}, {ls_adjustments.max():.6f}]')
        print(f'    Std dev: {ls_adjustments.std():.6f}')
        print(f'    RMS: {np.sqrt(np.mean(ls_adjustments**2)):.6f}')
